...
use model::Metadata;
pub use model::{ItemDocument, SearchHit};
use redb::{Database, ReadableDatabase, ReadableTable, TableDefinition};
use scan::{CatEntryKind, scan_cat};
...
"#,
        ...
        platformfs::LibraryFs::new(temp.path())
            .create_category_link(
                &types::CategoryPath::new("Wireless/RIS").unwrap(),
                &item_dir,
            )
            .unwrap();
    ...
    fn list_categories_includes_empty_cat_directories() {
        let temp = tempfile::tempdir().unwrap();
        platformfs::LibraryFs::new(temp.path()).ensure_layout().unwrap();
        platformfs::LibraryFs::new(temp.path())
            .create_category_dir(
                &types::CategoryPath::new("Inbox/New").unwrap(),
            )
            .unwrap();
        let db = StorageDb::open(temp.path()).unwrap();
