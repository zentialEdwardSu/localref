...
use model::{Event, ItemDocument, Metadata, SearchHit};
...
impl RestClient {
    ...
    /// Return one indexed item document.
    pub fn get_item(&self, item_id: &str) -> Result<ItemDocument, String> {
        self.get_json(&format!("/api/items/{}", encode_path(item_id)))
    }
    ...
    pub fn rename_category(
        ...
        self.post_json(
            ...
            &CategoryMoveRequest { from: from.into(), to: to.into() },
        )
    }

    /// Merge one category path into another.
    ...
    pub fn confirm_pending_import(
        ...
        self.post_json(
            &format!("/api/import/pending/{id}/confirm"),
            &PendingConfirmationRequest { categories },
        )
    }
    ...
    pub fn cancel_pending_import(
        &self,
        id: u64,
    ) -> Result<PendingImportSummary, String> {
        self.post_json(
            &format!("/api/import/pending/{id}/cancel"),
            &Value::Null,
        )
    }
